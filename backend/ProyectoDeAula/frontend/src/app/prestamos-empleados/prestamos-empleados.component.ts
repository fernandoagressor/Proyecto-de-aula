...
@Component({
  selector: 'app-prestamos-empleados',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './prestamos-empleados.component.html',
  styleUrls: ['./prestamos-empleados.component.css']
})
export class PrestamosEmpleadosComponent implements OnInit {
  ...
  cargarPrestamos(): void {
    ...
    const empresaId =
      ...
      this.usuarioLogueado?.empresa_id ||
    ...
    this.prestamoService.listarPorEmpresa(empresaId).subscribe({
      ...
      error: (err: any) => {
        ...
        console.error('Error al cargar préstamos de empresa', err);

        this.prestamos = [];

        this.mostrarToast(
          'No fue posible cargar los préstamos de empleados.',
          'error'
        );
  ...
  abonarPrestamo(id: number): void {
    ...
    this.prestamoService.abonarPrestamo(id, abono.toString()).subscribe({
      next: () => {
        ...
        this.mostrarToast(
          'Abono enviado correctamente. Queda pendiente de aprobación.',
          'success'
        );
  ...
  formatearDinero(valor: any): string {
  ...
  obtenerNombreEmpleado(prestamo: any): string {
    return (
      prestamo?.empleado?.nombre ||
      prestamo?.empleado?.nombres ||
      prestamo?.empleadoNombre ||
      'Empleado no asignado'
    );
  }

  obtenerInicialEmpleado(prestamo: any): string {
    const nombre = this.obtenerNombreEmpleado(prestamo);
    return nombre.charAt(0).toUpperCase();
  }

  obtenerCedulaEmpleado(prestamo: any): string {
    return (
      prestamo?.empleado?.cedula ||
      prestamo?.empleado?.documento ||
      prestamo?.empleadoCedula ||
      'Sin cédula'
    );
  }

  obtenerCargoEmpleado(prestamo: any): string {
    return (
      prestamo?.empleado?.cargo ||
      prestamo?.empleadoCargo ||
      'Cargo no registrado'
    );
  }
